...
- [ ] **Minor**: "over 15+ years" and bio copy trims, ~30% dead CSS in site.css, hero mailto ribbon competes with CTA, dots use `aria-pressed` instead of pagination semantics. (~~"Jame Rowe" typo~~ and ~~empty `<li>` slots in schedule~~ fixed 2026-07-30 — slots are now generated by the build.)
...
- [x] Built client content-editing system: `content/*.json` + `scripts/build.js` + GitHub Actions deploy + `.pages.yml` (Pages CMS) + `EDITING.md` — 2026-07-30
